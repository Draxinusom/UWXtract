/*************
	...
	UW1 PLAYER.DAT decoder/extractor
	...
	Todo:
		Might add some sort of item path like:
			...
			Pack (1)\Sack (3)\2
		Clean out logic around DATA\PLAYER.DAT as that is now excluded
*************/
#include "UWXtract.h"
...
// 0061-0062 -- Short value but just manually handling the overlap
	{
		...
		fprintf(
			...
			"%04X.2,"				// Offset
			"2,"					// Size
			"HallucinationLevel,"	// Property -- Assuming that's what shrooms is referring to?
			"%u\n",					// Value
		...
		fprintf(
			...
			"%04X.4,"				// Offset
			"6,"					// Size
			"IntoxicationLevel,"	// Property -- Intoxication sounds classier than drunk :P
			"%u\n",					// Value
			97 + FileByteOffset,								// Offset
		...
		fprintf(
			...
			"%04X.A,"			// Offset
			"2,"				// Size
			"GaramonBuried,"	// Property -- So far as I know this is treated like a boolean, not sure why there's 2 bits
			"%u\n",				// Value
			97 + FileByteOffset,		// Offset
...
// 00D2-00DB -- Unknown
	...
	/***
		Note:	00D2/3 are documented as number of inventory items held plus 1 but that doesn't match up at all
				with my test on a new save with the starting bag next to you the only item picked up having
...
// 00DC -- Vitality (Current/Unknown) -- Presumably used somewhere similar to 00B0 for mana
	fprintf(
		...
		"%04X,"						// Offset
		"1,"						// Size
		"Vitality_Current-Unknown,"	// Property
		"%u\n",						// Value
...
// Get number of items
	...
	int ItemOffset = IsEncrypted ? 312 : 197;
	int ItemCount = (ftell(SAVFile) - ItemOffset) / 8;
	fseek(SAVFile, ItemOffset, SEEK_SET);
